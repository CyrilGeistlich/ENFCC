...
netline <- ggplot(enfcc) +
  ...
  geom_text(x = 200000, y = 180000, label = "Net-Zero-Goal 2030", hjust = -0.1, vjust = -0.1, size = 4, angle = 37) +
...
netline_per_capita_zoom <- ggplot(enfcc_per_capita) +
  ...
  geom_text_repel(aes(weighted_projected_emissions_pop, tot_mit_reverse_pop, label = country), 
                  size = 2, force = 1, max.overlaps = 50) +
  scale_color_gradientn(colors = custom_colors, values = c(0, 0.003, 1), guide = "none") +
  theme(
    axis.text.x = element_text(angle = 0, vjust = 1, hjust = 1),
    axis.title = element_text(size = 12),
    plot.title = element_text(size = 14),
  ...
  scale_x_continuous(labels = label_number()) +
  scale_y_continuous(labels = label_number()) +
  coord_cartesian(xlim = c(0.0025, 0.008), ylim = c(-0.0025, 0.025)) +
  geom_text(x = 0.0065, y = 0.005, label = "Net-Zero-Goal 2030", hjust = 0, vjust = 0, size = 4, angle = 5)
...
index <- ggplot(enfcc_index, aes(country, score, fill = score >= 0)) +
...
#Arranve Score Ascending
enfcc_index <- enfcc_index %>%
  arrange(desc(score_per_capita)) %>%
  mutate(country = factor(country, levels = unique(country)))

index_per_capita <- ggplot(enfcc_index, aes(country, score_per_capita, fill = score_per_capita >= 0)) +
  geom_bar(stat = "identity", width = 0.8) +
  scale_fill_manual(values = c("gray", "lightgreen"),guide = "none") +
  labs(x = "Country", y = "Score") +
  theme(axis.text.x = element_text(angle = 45, vjust = 0.75, hjust = 1),
