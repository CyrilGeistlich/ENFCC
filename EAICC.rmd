...
netline <- ggplot(enfcc) +
  ...
  geom_text(x = 32000, y = 40000, label = "Net-Zero-Goal 2050", hjust = 0, vjust = 0, size = 4, angle = 71) +
  geom_text(x = 128000, y = 40000, label = "55%-Goal 2030", hjust = 0, vjust = 0, size = 4, angle = 71) +
...
scale_index <- function(score, baseline) {
  
  baseline <- baseline + min(score) # Move baseline
  score <- score + abs(min(score)) #So everything is positive, lowest score = 0
  
  baseline <- 1 + baseline/max(score) #scale baseline,
  score <- score/max(score) #scale everything into [0,1]
 
  score <- round(100 * (score - baseline),1) #Set 0 correctly, round, scale to 100, -100
  return(score)
}
...
# When comparing multiple years replace max(score) with absolute value
# scale_index <- function(score, baseline) {
#   zero <- ((baseline + abs(min(score)))/max(score)) #defines net zero boundary
...
# #This function is when all score values are negative
# scale_index <- function(score) {
#   zero <- ((0 + max(score))/max(score)) #defines net zero boundary
#   score <- ((score + max(score))/max(score))
# 
#   zero <- zero/max(score)
#   print(zero)
#   score <- round(-(score/max(score)-zero) * 100,1)
...
enfcc_per_capita  <- enfcc_per_capita |>
  mutate(score = min_distance_to_line(enfcc_per_capita$weighted_projected_emissions_pop, enfcc_per_capita$tot_mit_reverse_pop, -baseline_1990_capita))

enfcc_index <- data.frame(country = enfcc$country, score_per_capita = enfcc_per_capita$score,score = enfcc$score) 
enfcc_index$score <- scale_index(enfcc_index$score,baseline_1990)
enfcc_index$score_per_capita <- scale_index(enfcc_index$score_per_capita,baseline_1990_capita)
...
enfcc_index <- enfcc_index |>
  mutate(classification = cut(score, breaks = seq(-100, 100, by = 25),
                             labels = c("Atrocious", "Terrible", "Insufficient", "Slightly Insufficient", "OK", "Good", "Very Good", "Excellent"),
...
enfcc_index <- enfcc_index |>
  mutate(classification_per_capita = cut(score_per_capita, breaks = seq(-100, 100, by = 25),
                             labels = c("Atrocious", "Terrible", "Very Bad", "Bad", "OK", "Good", "Very Good", "Excellent"),
                             include.lowest = TRUE, right = FALSE))
```
...
color_palette <- c("#800000", "#B30000", "#E60000", "#FF9999")

# Create the plot
index <- ggplot(enfcc_index, aes(country, score, fill = classification)) +
...
index_per_capita <- ggplot(enfcc_index, aes(country, score_per_capita, fill = classification_per_capita)) +
  geom_bar(stat = "identity", width = 0.8) +
  scale_fill_manual(values = color_palette) +
  labs(x = "Country", y = "Score") +
  theme(axis.text.x = element_text(angle = 45, vjust = 0.75, hjust = 1),
