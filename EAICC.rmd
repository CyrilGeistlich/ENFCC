...
netline <- ggplot(enfcc) +
  ...
  geom_text(x = 80000, y = 70000, label = "Net-Zero-Goal 2030", hjust = 0, vjust = 0, size = 4, angle = 0) +
  geom_text_repel(aes(weighted_projected_emissions, tot_mit_reverse, label = country), size = 2, force = 1, max.overlaps = 50) +
  scale_color_gradientn(colors = custom_colors, values = c(0, 1, 2), guide = "none") +
  theme(
...
netline_per_capita <- ggplot(enfcc_per_capita) +
  ...
  geom_abline(slope = 1, linetype = "dashed") +
  geom_text(x = 7, y = 5.5, label = "Net-Zero-Goal 2030", hjust = 0, vjust = 0, size = 4, angle = 18) +
...
netline_zoom <- ggplot(enfcc) +
  ...
  ggtitle("European Net Footprint for Climate Change (ENFCC) per Country") +
  geom_abline(slope = 1, linetype = "dashed") +
  geom_text(x = 100000, y = 85000, label = "Net-Zero-Goal 2030", hjust = -0.1, vjust = -0.1, size = 4, angle = 37) +
  geom_text_repel(aes(weighted_projected_emissions, tot_mit_reverse, label = country), size = 2, force = 1, max.overlaps = 50) +
  scale_color_gradientn(colors = custom_colors, values = c(0, 1, 2), guide = "none") +
  theme(
    axis.text.x = element_text(angle = 0, vjust = 0.75),
...
netline_per_capita_zoom <- ggplot(enfcc_per_capita) +
  ...
  geom_text(x = 7.5, y = 6.5, label = "Net-Zero-Goal 2030", hjust = 0, vjust = 0, size = 4, angle = 19)
...
enfcc <- enfcc |>
  mutate(score = min_distance_to_line(weighted_projected_emissions, tot_mit_reverse))

enfcc_per_capita  <- enfcc_per_capita |>
  mutate(score = min_distance_to_line(enfcc_per_capita$weighted_projected_emissions_pop, enfcc_per_capita$tot_mit_reverse_pop))
