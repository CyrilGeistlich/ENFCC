...
```{r}
#GHG Projection Total inc. int. Aviation excl. LULUCF for WAM-Scario (with additional measures)
# TODO Discuss if either WEM or WAM should be used. 
...
emission <- emission %>% mutate(projeced_emission_pop=projection/pop*1000) # TODO(?) [kt CO2eq per capita] /!\ NOT WEIGHTED /!\
```
...
# TODO /!\ Check if all columns that should be summed up are included here /!\ 
all_data <- all_data %>% mutate(netto = rowSums(.[c(2,6, 10, 11, 12)],na.rm=TRUE))
```
...
- Gewichtung anpassen
- Mit min-max -> müsste Referenzperiode wählen um so zu vergleichen (gleiche min, max values über die Jahre) 
so gewählt damit aber momentan reproduzierbar
