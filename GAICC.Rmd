...
protected_areas$`protected_areas_2022_percent` <- as.numeric(protected_areas$`protected_areas_2022_percent`)
...
scale_values_negative <- function(x) {
...
```{r calc index}
#Sum of emissions % percent
WAICC$emissions_per_capita_percent <- WAICC$emissions_per_capita/sum(WAICC$emissions_per_capita) * 100
...
columns_to_scale_n <- c("municipal_solid_waste_per_capita", "emissions_per_capita")

#positive rescale columns between -1 and 1
for (col in columns_to_scale_p) {
  new_col_name <- paste0(col, "_scaled")
  WAICC[, new_col_name] <- scale_values_positive(WAICC[, col])
}

for (col in columns_to_scale_n) {
  new_col_name <- paste0(col, "_scaled")
  WAICC[, new_col_name] <- scale_values_negative(WAICC[, col])
}
```
...
WAICC <- WAICC |>
  ...
  mutate(dim_2 = (political_engagement_score_scaled + 2 * investment_per_capita_scaled +  protected_areas_2022_percent_scaled)/4)
...
write.csv(WAICC, "/Users/jonguler/Library/Mobile Documents/com~apple~CloudDocs/Uni/Semester/8. Semester/GEO 888/Project/WAICC.csv")
